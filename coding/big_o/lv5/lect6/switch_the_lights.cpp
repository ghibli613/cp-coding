...
//   - mask will help us to know which lamps are turned on, the ith bit in the mask is set to 1 if the ith lamp is turned on,
...
int dp(int i, int mask)
{
    ...
    { // we reached the end
        if (mask)
        { // invalid case, some lamps are turned on
            return 1e5;
        }
