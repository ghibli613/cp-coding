...
void solve(int n, int m)
{
    ...
    for(int i = 0; i < m; i++)
    {
        do
        {
            ...
            if(multi_task_end[i] > 1e6) multi_task_end[i] = 1e6 + 1;
    ...
    for(int i = 0; i <= 1e6; i++)
    {
        ...
        {
            if(calendar[i][0] == flag) flag = calendar[i][1];
            else if (calendar[i][1] == flag) flag = calendar[i][0];
            else
            {
                cout << "CONFLICT\n";
                return;
